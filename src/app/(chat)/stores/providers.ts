...
export const useProvidersStore = create<ProvidersState>()(
  devtools(
    (set, get) => ({
      ...
      // Set selected model
      setSelectedModel: (model: AvailableModel | null) =>
      ...
      isDataStale: () => {
        const state = get()
        return (
          !isCacheValid(state.lastFetched.providers) ||
          !isCacheValid(state.lastFetched.userSettings)
        )
      },
    }),
    {
