...
import { showStreamError } from '@/features/chat/lib/toast-utils'
import type {
...
export const useChatSession = (options: UseChatSessionOptions): UseChatSessionResult => {
  ...
  const fetchConversation = useCallback(async () => {
    ...
    setError(null)
    try {
      const response = await fetch(`/chat/api/${conversationId}`, {
  ...
  const sendMessage = useCallback(
    async (data: PostMessageRequest) => {
      ...
      try {
        const response = await fetch(`/chat/api/${conversationId}/messages`, {
          method: 'POST',
          headers: {
